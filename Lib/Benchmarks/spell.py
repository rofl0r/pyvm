...
# IN ORDER TO RUN THIS YOU'LL NEED THE FILE "big.txt" FROM http://norvig.com
# OR SOME OTHER BIG TEXT WITH LOTS OF WORDS, AND PLACE IT INTO DATA/big.txt

#DEJAVU
'''
{
'NAME':"Spellcheck",
'DESC':"Peter Norvig's Spelling Corrector",
'GROUP':'real-bench',
'CMPOUT':1,
'ARGS':"SEM",
'BARGS':"",
...
'''
# pyvm: we are not using a defaultdict. in the original spell.py, the
# defaultdict has two uses:
# 1) as a counter for the train-set
# 2) so that new words have an appearance of 1
# The first use is avoided because in pyvm we have a custom counter
# class. We don't use it here to be fair:)
# The second use is avoided through code refactoring (just return the
# word if unknown).
...
def correct(word):
    ...
    if not candidates:
	return word
    return max ([(NWORDS [w],w) for w in candidates])[1]
...
testssem = { 'access': 'acess', 'accessing': 'accesing', 'accommodation':
'accomodation acommodation acomodation', 'account': 'acount', 'address':
'adress adres', 'addressable': 'addresable'
}

if __name__ == '__main__':
    import sys
    if "SEM" in sys.argv:
        spelltest(testssem, verbose=True)
    else:
