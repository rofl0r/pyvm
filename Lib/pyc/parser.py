...
PYVM = 'pyvm' in sys.copyright

def intern_if_all_chars (STR):
	if len (STR) > 15 or not STR.isalpha ():
		return STR
	return GCO.Intern (STR)
...
def source_generator (source):
	for LINENO, line in enumerate1 (source.split ('\n')):
		yield LINENO, line+'\n'
...
def count_idnt (line):
	cnt = 0
	for i in line:
		if i == '\t': cnt = (cnt + 8) & ~7#cnt = ((cnt + 8) / 8) * 8
		elif i == ' ': cnt += 1
		else: return cnt
...
class Lexer:
	...
	def tokenize (self, line, LINENO, symtable=symtable, evalconst=evalconst,
			TokenizeEx=R.TokenizeEx, Evaluable=R.Evaluable,
			CNESTS = {')':'(', ']':'[', '}':'{'}):
		nestor = []
		tokens = []
		...
		while 1:
			...
			while tok and tok [-1][-1] == '\n':
				if tok [-1][0] == '\\':
					...
					s = tok.pop ()
					EndMulti = self.R.EndMultiLine [self.R.Quotes (s).group (1)]
					for LINENO, i in self.SOURCEGEN:
						q = EndMulti (i)
						if not q:
							s += i
						else:
							tok.append (s + q.group (1))
							tok += TokenizeEx (q.group (2))
							break
					else:
			...
			for i in tok:
				if Evaluable (i):
					i = evalconst (i)
					if tokens and tokens [-1][0] == VAL_BASE:
						tokens [-1] = (VAL_BASE, tokens [-1][1] + i, LINENO)
					else:
						if type (i) is str:
							i = intern_if_all_chars (i)
						tokens.append ((VAL_BASE, i))
						#tokens.append ((VAL_BASE, i, LINENO))
				elif i in symtable:
					tokens.append ((symtable [i], LINENO))
					if i in '([{':
						nestor.append (i)
					elif i in ')]}':
						if not nestor or nestor.pop () != CNESTS [i]:
							raise SyntaxError ("Bad `%s` nesting near line %i" % (i, LINENO))
				else:
					tokens.append ((SYM_BASE, intern (i), LINENO))
			if not nestor:
				break
			LINENO, line = self.LG2.next ()
		return tokens
...
class DFA_parser:
	...
	'pyc::membertype self.pending list'
...
#		self.flush (ENDALL_PRI)
		...
		self.DFA0, self.DFATRAIL, self.DFALINE, self.st0, self.ingen, self.tnest, self.forfail = self.dfas.pop ()
	...
	def do_with (self, lno):
		self.stack [-2:] = [ast.With (self.stack [-2], None, self.stack [-1])]
