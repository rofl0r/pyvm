##  Module symbol-table analysis
##  Based on symbol.py from python 2.4
##
...
from gco	import GCO
from optimizer	import make_copy, rename
...
class Scope:
	...
	def __init__(self, name, stype, module = None):
		...
		self.nested = self.generator = self.this = None
	...
	def handle_children(self):
		for child in self.children:
			...
			for forced, name in Globals:
				child.force_global (forced, name)

	def force_global(self, forced, name):
		...
		# free.
		if not forced:
			self.maybe_builtins.append (name)
		self.globals.add (name)
	...
	def add_frees(self, names):
		...
		#
		# (btw: something declared global with the "global" keyword and
		# something that's just undefined and thus may be global or builtin,
		# are different. The latter can be subject to "marshal builtins")
		child_globals = []
		for name in names:
			sc = self.check_name(name)
			if self.nested:
				if sc == 'SC_UNKNOWN' or sc == 'SC_FREE' or self.stype == SCOPE_CLASS:
					self.frees.add (name)
				elif sc == 'SC_GLOBAL':
					child_globals.append ((True, name))
				elif self.stype in (SCOPE_FUNC, SCOPE_GEXP) and sc == 'SC_LOCAL':
					self.cells.add (name)
				elif sc != 'SC_CELL':
					child_globals.append ((False, name))
			else:
				if sc == 'SC_LOCAL':
					self.cells.add (name)
				elif sc != 'SC_CELL':
...
class SymbolVisitor:
	...
	def visitModule(self, node):
		...
		scope.Classes = node.Classes = []
		self.BoundNames = node.BoundNames
	...
	def visitFunction(self, node, parent):
		...
		if not scope.nested and not scope.children and not scope.frees:
			s = scope.uses - set (scope.defs.keys ()) - scope.frees - scope.globals - scope.params
			b = []
			for n in s:
				if n in self.BoundNames:
					b.append (n)
			##		print "use bound name:", n
			if b:
				scope.bound = sorted (tuple (b))

	def visitGenExpr(self, node, parent):
		scope = GenExprScope(self.module)
		if parent.nested or parent.stype == SCOPE_FUNC or parent.stype == SCOPE_GEXP:
			scope.nested = 1
