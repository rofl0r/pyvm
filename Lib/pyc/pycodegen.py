##  pyvm boot script
##  Based on pycodegen.py from python 2.4
##
...
try:
	...
	from consts import CO_VARARGS, CO_VARKEYWORDS, CO_NEWLOCALS, CO_NESTED, CO_GENERATOR, CO_GENERATOR_ALLOWED, CO_FUTURE_DIVISION
...
EXIT_NODES = set ([ast.Return, ast.Raise, ast.Break, ast.Continue])

def last_exit (node):
	if isinstance (node, ast.Stmt) and node.nodes:
		last = node.nodes [-1]
		if last.__class__ in EXIT_NODES:
			return True
...
class CodeGenerator:
	...
	'pyc::membertype self.setups list'
	...
	def _setupGraphDelegation(self):
		...
		self.setprop = self.graph.setprop
		self.newBlock = self.graph.newBlock
		self.startBlock = self.graph.startBlock
		self.nextBlock = self.graph.nextBlock
		self.startNewBlock = self.graph.startNewBlock
		self.setDocstring = self.graph.setDocstring
		self.graph.control_blocks = self.setups
	...
	def _visitFuncOrLambda(self, node, isLambda=0, hasReturn=0):
		...
		elif gen.scope.bound:
			for name in gen.scope.bound:
	...
	def visitIf(self, node):
		...
		for test, suite in node.tests:
			...
			self.emit('POP_TOP')
			if (test is node.tests [-1][0] and not node.else_ and
			isinstance (suite.nodes [-1], ast.Discard)):
				# if there is no 'else' part and the last opcode of the body
				# is a POP_TOP, do not emit it. Instead skip the JUMP_FORWARD
				# after the final POP_TOP.
				suite.nodes [-1] = suite.nodes [-1].expr
				emitj = False

			self.visit(suite)

			if emitj and not last_exit (suite):
	...
	def visitWhile(self, node):
		...
		self.nextBlock(loop)
		self.setups.append(('LOOP', loop, after))

		self.visitBoolean (node.test, 'JUMP_IF_FALSE', else_)
	...
	def visitDoWhile(self, node):
		...
		#	POP_TOP
		#
		# which would make iteration faster. But JUMP_IF is a relative jump
		# and relative jumps are supposed to jump forward only. Considering
		# that do-while is rare, we can leave it as it is.
		#
		loop = self.newBlock()
		else_ = self.newBlock()
		after = self.newBlock()
	...
	def visitFor(self, node):
		...
		if need_setup:
			self.emit ('SETUP_LOOP', after)
		self.setups.append(('LOOP', start, after))
	...
	def visitBreak(self, node):
		...
		self.setprop ("has_break", True)

	def visitContinue(self, node):
		if not self.setups:
			raise SyntaxError, "'continue' outside loop (%s, %d)" % (node.filename, node.lineno)
		kind, block, after = self.setups [-1]

		in_eh = False
		if kind == 'EXCEPT_HANDLER':
			in_eh = True
			for kind, block, after in reversed (self.setups):
				if kind != 'EXCEPT_HANDLER':
					break
			else:
				raise SyntaxError, "'continue' outside loop (%s, %d)" % (node.filename, node.lineno)
...
#			self.startNewBlock()
			...
			while top > 0:
				top = top - 1
				kind, loop_block, after = self.setups[top]
				if kind == 'LOOP':
					break
			if kind != 'LOOP':
				raise SyntaxError, "'continue' outside loop (%s, %d)" % (node.filename, node.lineno)
	...
	def visitListComp(self, node):
		...
		for start, cont, anchor in stack:
			...
			## In official this is a `startBlock`. I've switched
			## to `nextBlock` so this doesn't point to "exit" (?)
			##self.nextBlock(anchor)
	...
	def visitTryExcept(self, node):
		...
		self.nextBlock(body)
		self.setups.append(('EXCEPT', body, None))
		...
		self.setups.append (('EXCEPT_HANDLER', None, None))
	...
	def visitTryFinally(self, node):
		...
		self.nextBlock(body)
		self.setups.append(('TRY_FINALLY', body, None))
		...
		self.setups.append(('END_FINALLY', final, None))
		self.visit(node.final)
		self.emit('END_FINALLY')
		self.setups.pop()

	def visitExcOr (self, node):
		# This is the ?? operator of the whitespaceless.
		# It appears to work. The worrysome thing is that
		# in normal try-except operation, the stack before
		# the try-except and after it are at the same top.
		# with this construct a value leaks to the stack top.

		# Note that the sys.exc_info is released early!
		
		try:
			n1, n2 = node.nodes
		except:
			print "?? works with only two nodes"
			raise SyntaxError
		self.graph.enterTry ()
		body = self.newBlock()
		self.graph.leaveTry ()
	...
	def visitGetattr(self, node):
		if type (node.attrname) is not str:
			# dynamic attributes. we marshal builtin `getattr`
			# and thus this is for pyvm only
			self.emit ('LOAD_CONST', getattr)
			self.visit (node.expr)
			self.visit (node.attrname)
	...
	def visitAugGetattr(self, node, mode):
		if type (node.attrname) is not str:
			# XXXX: We evaluate the expressions twice!
			# once for load and once for store, while
			# the original AugGetattr evaluates the
			# base expression only once.  For example
			#  x [1][2].y = 3
			# "x [1][2]" is evaluated once
			# We need some complex stack rotatin'
			if mode == "load":
				self.emit ('LOAD_CONST', getattr)
				self.visit (node.expr)
				self.visit (node.attrname)
...
class FunctionCodeGenerator(NestedScopeMixin, AbstractFunctionCode,
	...
	def __init__ (self, func, isLambda, mod):
		...
		self.graph.setBoundFreeVars (self.scope.bound)
...
class BreakFinder:
	...
	def visitWhile (self, node):
		if node.else_:
			self.visit (node.else_)
	visitFor = visitDoWhile = visitWhile
