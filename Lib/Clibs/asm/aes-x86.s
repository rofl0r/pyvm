# This is an implementation of AES for x86 from the OpenSSL project
# (version 0.9.8d). It is about 2 times faster than the C version.
# HOW-EVER, two things:
#	- this code appears to be PIC. We do not want PIC for pelf.
#	  should remove the PICness.
#	- there seems to be a real opportunity for MMX optimization
#	  in CBC encrypt.  If the output of {en/de}crypt_block is
#	  stored in two MMX registers good stuff can happen.
#	- In the meantime, OpenSSL improves. it should be possible
#	  to do the above two things _and_ be in sync with upstream.
# NOTE:
#	it seems that the cbc_encrypt function writes the output in-place
#	over the input, even if two different buffers are passed to
#	inptr/outptr; not sure how it works.  The AES.pe file has been
#	adapted to always ask for in-place operation, but beware.
#
# Original Copyright:
# ====================================================================
...
# ====================================================================
# Modifications by sxanth:
#	added public ABI string
#	made encrypt/decrypt functions and Te/Td tables private
#	renamed the AES_* exported functions without the AES_ prefix
...
.L029skip_dzero:
	...
	ret
.L_cbc_encrypt_end:
.size	cbc_encrypt,.L_cbc_encrypt_end-cbc_encrypt
.ident	"cbc_encrypt"
.text
.globl	set_encrypt_key
.type	set_encrypt_key,@function
.align	16
set_encrypt_key:
	pushl	%ebp
	pushl	%ebx
...
.L035exit:
	...
	popl	%ebp
	ret
.L_set_encrypt_key_end:
.size	set_encrypt_key,.L_set_encrypt_key_end-set_encrypt_key
.ident	"set_encrypt_key"
.text
.globl	set_decrypt_key
.type	set_decrypt_key,@function
.align	16
set_decrypt_key:
	movl	4(%esp),	%eax
	movl	8(%esp),	%ecx
...
.L047permute:
	...
	popl	%ebp
	ret
.L_set_decrypt_key_end:
.size	set_decrypt_key,.L_set_decrypt_key_end-set_decrypt_key
.ident	"set_decrypt_key"
