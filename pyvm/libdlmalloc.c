...
/*
	...
	- ifdef'd out:
	  ...
	  independents* later...
	- MALLOC_FAILURE_ACTION is no-op, and iow we don't set errno.

tunnings:
 MXFAST changed from 64 to 16
	Most small requests are handled by seg-malloc. Changing MXFAST to 0 was slower and
	used more memory. `16` seemed a little better than 64.

 MMAP_THRESHOLD changed from 256k to 128k
	reduced overall memory of www for a huge page about 5%.
	using 100k, had the same amount but the program was a tad slower.

*/
//#define WANT_MALLINFO
...
#define MALLOC_COPY(dest, src, nbytes)  __builtin_memcpy(dest, src, nbytes)
#define MALLOC_ZERO(dest, nbytes)       __builtin_memset(dest, 0,   nbytes)
...
#define check_malloc_state() pyvm_checkstate()
...
*/
extern void pyvm_memory_manager (unsigned int);

static void pyvm_checkstate ()
{
  mstate av = get_malloc_state();
  pyvm_memory_manager (av->mmapped_mem + av->sbrked_mem);
}
