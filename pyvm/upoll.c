...
int upoll(struct pollfd *fds, int nfds, long long int timeout);

#ifndef HAVE_PPOLL
/* poll() emulation with select().
   the following code was taken from uClibc, which in turn integrated
   it from glibc. Changed for pyvm to pass microsecond timeouts.
   Original copyright right below  */
...
int upoll(struct pollfd *fds, int nfds, long long int timeout)
{
	if (timeout != -1) {
		...
		tt.tv_nsec = 1000 * (timeout % 1000000);
		return ppoll (fds, nfds, &tt, 0);
	}
